"""Added talents test models

Revision ID: 6a2a9a4d509f
Revises: 
Create Date: 2025-03-28 08:41:24.374221
...
def upgrade() -> None:
    ...
    sa.Enum('paid', 'not_paid', name='invoicestatusenum').create(op.get_bind())
    ...
    sa.Column('status', postgresql.ENUM('paid', 'not_paid', name='invoicestatusenum', create_type=False), nullable=False),
    ...
    op.create_index(op.f('ix_talents_invoice_id'), 'invoice', ['id'], unique=False, schema='talents')
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_talents_invoice_id'), table_name='invoice', schema='talents')
    op.drop_table('invoice', schema='talents')
    ...
    sa.Enum('paid', 'not_paid', name='invoicestatusenum').drop(op.get_bind())
    # ### end Alembic commands ###
